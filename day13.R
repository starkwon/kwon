...
plot(국어, type="o", col="blue")
lines(수학, type="o", pch=16, lty=2, col="red") #pch는 점의 형태    
title(main="성적그래프", col.main="red", font.main=4)

국어 <- c(4,7,6,8,5,5,9,10,4,10)
par(mar=c(1,1,1,1), mfrow=c(4,2))# 마진단위 1로 지정, mfrow는 행렬 형태로 지정
...
axis(2, at=c(0,2,4,6,8,10))  # y축 추가(위치 지정 아규먼트)
...
legend(8, 4, c("국어","수학"), cex=0.8, col=c("blue","red"), pch=c(21,16), lty=c(1,2))  
...
?plot
plot(국어, 수학)
plot(국어~수학) #수학 성적에 기반한 국어 성적이 어떻게 되니?
...
# 우측에 범례가 들어갈 여백을 확보(모자르면 마진영역 확장<xpd=T> 해서 그려라)
